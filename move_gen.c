    ...
    if (UCI_OPTION_CHESS_960) /* Attend Chess960 */
        ...
        if (CastleOO)
            ...
            if (T & POSITION->OccupiedBW)
                goto NO_OO;
            MoveAdd(LIST,
              FlagOO | (CHESS_960_KR_FILE + 8 * NUMBER_RANK1) | ((CHESS_960_KING_FILE + 8 * NUMBER_RANK1) << 6),
        ...
        if (CastleOOO)
            ...
            if (T & POSITION->OccupiedBW)
                goto NO_OOO;
            MoveAdd(LIST,
              FlagOO | (CHESS_960_QR_FILE + 8 * NUMBER_RANK1) | ((CHESS_960_KING_FILE + 8 * NUMBER_RANK1) << 6),
...
typeMoveList* MyQuietChecks( typePOS* POSITION, typeMoveList* LIST, uint64 mask )
    ...
    opks = OppKingSq;
    T = CAPTURE_LEFT &(~BITBOARD_EIGHTH_RANK) & mask & OppOccupied & MyAttackedPawns[opks];
    while (T)
        {
        to = BSF(T);
    ...
    T = CAPTURE_RIGHT &(~BITBOARD_EIGHTH_RANK) & mask & OppOccupied & MyAttackedPawns[opks];
    while (T)
        {
        to = BSF(T);
    ...
    if (BitboardOppK & FOURTH_EIGHTH_RANK_NOh && POSITION->sq[BACK_RIGHT(opks)] == 0)
        {
        ...
        if (RANK(opks) == NUMBER_RANK5 && POSITION->sq[BACK_RIGHT2(opks)] == 0
    ...
    if (BitboardOppK & FOURTH_EIGHTH_RANK_NOa && POSITION->sq[BACK_LEFT(opks)] == 0)
        {
        ...
        if (RANK(opks) == NUMBER_RANK5 && POSITION->sq[BACK_LEFT2(opks)] == 0
