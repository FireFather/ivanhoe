...
void InitBitboards( typePOS* POSITION )
    ...
    if (POPCNT(wBitboardQ) > 1 || POPCNT(bBitboardQ) > 1 || POPCNT(wBitboardR) > 2 || POPCNT(bBitboardR) > 2
      || POPCNT(wBitboardBL) > 1 || POPCNT(bBitboardBL) > 1 || POPCNT(wBitboardN) > 2 || POPCNT(bBitboardN) > 2
    ...
    if (UCI_OPTION_CHESS_960)
        {
        if (WhiteOO && (POSITION->wKsq != CHESS_960_KING_FILE || !(wBitboardR &SqSet[CHESS_960_KR_FILE])))
            BAD("arrocco illegale");
        if (WhiteOOO && (POSITION->wKsq != CHESS_960_KING_FILE || !(wBitboardR &SqSet[CHESS_960_QR_FILE])))
            BAD("arrocco illegale");
        if (BlackOO && (POSITION->bKsq != (CHESS_960_KING_FILE + 070) || !(bBitboardR &SqSet[CHESS_960_KR_FILE + 070])))
            BAD("arrocco illegale");
        if (BlackOOO
          && (POSITION->bKsq != (CHESS_960_KING_FILE + 070) || !(bBitboardR &SqSet[CHESS_960_QR_FILE + 070])))
            BAD("arrocco illegale");
        }
    else
        {
    ...
    POSITION->DYN->PAWN_HASH ^=
      ZobristCastling[POSITION->DYN->oo] ^ TWEAK ^ Zobrist(wEnumK, POSITION->wKsq) ^ Zobrist(bEnumK, POSITION->bKsq);
    if (POSITION->wtm)
