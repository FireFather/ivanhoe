...
void MC_StoreResult (type_Monte_Carlo * MC, int moveno, int value)
{
  ...
  for (int i = 0; i <= 2; i++)
	{
	  if (MC->SCORE[i] > BESTSCORE)
		{
		BESTSCORE = MC->SCORE[i];
//		SEND("moveno %d score %d", moveno, BESTSCORE);
		SEND("move %s score %d", Notate(MC->SEARCH[moveno].move, string), BESTSCORE);
		SEND("\n");
	    }
//	  MC->SEARCH[moveno].move
